...
function vk_add_sns_menu() {
	$capability_required = ex_unit_get_capability_required();
	$custom_page = add_submenu_page(
		...
		'vk_sns_options',			// ユニークなこのサブメニューページの識別子
		'add_vk_sns_options'		// メニューページのコンテンツを出力する関数
	);
	if ( ! $custom_page )
	return;
}
add_action( 'admin_menu', 'vk_add_sns_menu' );
...
function bizVektorAddSnsStyle(){
	...
	// wp_enqueue_style( 'ex_unit_css_path', $cssPath , false, '2013-05-13b');
